        ...
        private void StartNewTournaments()
        {
            ...
            foreach (Tournament tournament in tournamentsToStart)
            {
                ...
                for (int i = 0; i < tournamentUsers.Count; i++)
                {
                    ...
                    {
                        res.Add(new Pair<string, string>(tournamentUsers[i], tournamentUsers[j]));
                    }
                }
            }

            // tournamentsToStart
            //     .Where(tournament => (Tournament.TournamentType) tournament.Type == Tournament.TournamentType.MATCHUPS)
            //     .Select(tournament => tournament.Contests)
        }
