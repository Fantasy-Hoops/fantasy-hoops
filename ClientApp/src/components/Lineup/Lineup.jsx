...
export class Lineup extends Component {
  ...
  async componentWillMount() {
    ...
    await fetch(`http://http://68.183.213.191:5001/api/player`)
      .then(res => {
        return res.json()
      })
      .then(res => {
  ...
  async componentDidUpdate() {
    ...
    if (!this.state.loadedPlayers && this.state.players) {
      const user = parse();
      await fetch(`http://http://68.183.213.191:5001/api/lineup/${user.id}`)
        .then(res => {
          return res.json()
        })
        .then(res => {
  ...
  async showModal(player) {
    ...
    await fetch(`http://http://68.183.213.191:5001/api/stats/${player.id}`)
      .then(res => res.json())
      .then(res => {
        this.setState({
          stats: res,
