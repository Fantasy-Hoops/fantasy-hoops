  ...
  render() {
    if (this.props.status > 0) {
      ...
      if (!this.props.player.injuryStatus.toLowerCase().includes("active"))
        injuryBadge = <div className={"PlayerCard__player-injury-badge " + injuryStatus}>{this.props.player.injuryStatus}</div>

      const image = <Img
        onClick={this.props.status === 2 ? this.filter : undefined}
        className={`PlayerCard__player-img PlayerCard__player-img${this.props.status === 2 ? '--lineup' : ''}`}
