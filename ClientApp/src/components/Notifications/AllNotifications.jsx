...
export class AllNotifications extends Component {
  ...
  async componentWillMount() {
    await fetch(`http://localhost:44307/api/lineup/nextGame`)
      .then(res => {
        return res.json()
      })
      .then(res => {
    ...
    await fetch(`http://localhost:44307/api/notification/${user.id}?count=10`)
      .then(res => {
        return res.json()
      })
      .then(res => {
  ...
  async toggleNotification(notification) {
    ...
    await fetch('http://localhost:44307/api/notification/toggle', {
      method: 'POST',
      headers: {
    ...
    await fetch(`http://localhost:44307/api/notification/${user.id}?count=${this.state.userNotifications.length}`)
      .then(res => {
        return res.json()
      })
      .then(res => {
  ...
  async loadMore() {
    ...
    await fetch(`http://localhost:44307/api/notification/${user.id}?start=${this.state.userNotifications.length}&count=${LOAD_COUNT}`)
      .then(res => {
        return res.json()
      })
      .then(res => {
